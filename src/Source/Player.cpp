...
void Player::set_player_state_reloading() {
    if(mag_ammo == 0 && !player_state.reloading && mag_ammo != MAG_CAPACITY && leftover_ammo > 0){
...
void Player::transfer_data_from(const PlayerObject& object){
    ...
    this->shoot_timer = object.reload_timer;
...
void Player::transfer_data_to(PlayerObject& object){
    ...
    object.reload_timer = this->shoot_timer;
...
void Player::transfer_data_to(PlayerEvent &player_event, bool gained_focus) {
    ...
    if(gained_focus){
        ...
        //std::cout << "SPRITE ROT: " << sprite.getRotation() << "\n";
        ...
        //std::cout << "rot_whole_player_event: " << static_cast<int32>(player_event.rotation_whole_number) << "rot_decimal_player_event: " << static_cast<int32>(player_event.rotation_decimal_number) << "\n";
        //std::cout << "rot_whole: " << sprite.getRotation() << "rot_decimal: " << static_cast<uint32>((sprite.getRotation() - static_cast<uint32>(sprite.getRotation())) * 1000) << "\n";
...
void Player::verify_and_assign_player_state(PlayerEvent &player_event){
    ...
    if(player_event.shoot_button_pressed){
        if(can_shoot()){
    ...
    if(player_event.event_button_pressed){
        if(team == TEAM_T){
            player_state.planting_bomb = true;
        } else {
            player_state.defusing_bomb = true;
        }
    }

    if(player_event.reload_button_pressed){
        set_player_state_reloading();
    }

    if(player_event.drop_button_pressed){
...
void Player::plant_bomb(sf::Sprite &plant_area, std::pair<bool, sf::Sprite> &bomb, Animation &plant_animation){
...
void Player::defuse_bomb(std::pair<bool, sf::Sprite> &bomb, Animation &defuse_animation){
    if(team == TEAM_CT){
